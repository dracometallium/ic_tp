...
\pagestyle{fancy}
\lhead{\footnotesize Trabajo práctico de Laboratorio - Introducción al shell del sistema GNU/LINUX}
...
El concepto de directorios (comúnmente llamados ``carpetas'') y archivos nos
es hoy en día familiar. En general quienes utilizan el sistema se manejan
...
archivos en el intérprete, con rutas absolutas y relativas. Cuando utilizamos
la forma absoluta se escribe todo el camino desde la carpeta inicial (en
\cw{Linux} se denota con el símbolo \cw{``/''}) hasta la ubicación deseada.
Ejemplos de uso absoluto: \cw{/home/usuarioX/trabajo1}, \cw{/etc}, \cw{/tmp},
etc.
...
\begin{enumerate}[resume]
    ...
    \item Para mayor dramatismo, ejecute lo siguiente: \cw{cat archivo.txt |
        cowsay -n | lolcat} (todos los comandos en la misma linea, separados
        por el carácter ``\cw{|}'') ¿Qué hace cada uno de los comandos de la
        secuencia?
...
\begin{enumerate}[resume]
    ...
    \item Cree un archivo con nombre \cw{dance.sh} y el siguiente contenido.
...
while true; do
    ...
    \item Verifique que el contenido del archivo es correcto revisando la
        salida del comando \cw{cat danse.sh}.
    ...
    \item Abra otra ventana (sin cerrar aquella donde se esta ejecutando el
        programa \cw{dance.sh}) y utilizando el comando \cw{ps a} encuentre el
...
echo "El resultado de sumar 3 unas $max veces es: $num"
    ...
    \item Verifique que el contenido del archivo se almaceno correctamente
        ejecutando el comando \cw{cat calcular.sh}.
...
int main(int carg, char **varg)
{
    ...
    \item Verifique que el contenido del archivo se almaceno correctamente
        ejecutando el comando \cw{cat calcular.c}.

    \item Cree un archivo objeto optimizado con el comando \cw{gcc -c -O3
        calcular.c } (\cw{-O} es la letra ``O'' mayúscula) ¿Qué son los
        archivos objeto?

    \item Cree un archivo ejecutable con el comando \cw{gcc calcular.o -o
    ...
    \item \cw{GCC} tiene una función que permite convertir el programa en
        lenguaje \emph{C} a ensamblador, para permitirle al programador saber
        exactamente a que instrucciones se traducirá el programa.  Cree un
        archivo ensamblador optimizado con el comando \cw{gcc -S -O3
        calcular.c } (\cw{-O} es la letra ``O'' mayúscula). Puede inspecionar
        el contenido del archivo \cw{calcular.s} (con \cw{less} o similaar).
...
\begin{enumerate}[resume]
    ...
    \item Modifique ambos programas para que sumen el número 42 en lugar de 3.
        Verifique que el cambio se realizo con éxito sumando 100 veces.
...
\subsection{Direcciones \emph{IP} y enrutamiento}

\begin{enumerate}[resume]

    \item Utilice el comando \cw{dig} para obtener la dirección \emph{IP} del
        servidor cuyo nombre de dominio es ``\url{uncoma.edu.ar}''.

    \item Verifique que sus mensajes pueden llegar hasta el servidor, usando
        el comando\\
        \cw{ping -n uncoma.edu.ar} ¿Qué otra información provee el comando?

    \item ¿Existe alguna diferencia entre ejecutar el comando \cw{ping}
        utilizando la dirección \emph{IP} en lugar del nombre de dominio del
        servidor? ¿Cuál es la utilidad de los nombres de dominio?

    \item El comando \cw{traceroute} permite obtener información sobre los
        routers por los que pasaran los paquetes en su camino hasta el
        servidor destino ¿Cuántos nodos hay entre su computadora y el servidor
        de ``\url{uncoma.edu.ar}''?
...
\begin{enumerate}[resume]
    ...
    \item El comando \cw{netcat} nos permitirá recibir y enviar bytes a
        través de la red. Abra dos consolas, en una  ejecute el comando
        \cw{netcat -l -p 8080}, y en la otra \cw{netcat localhost 8080}. Si
        escribe algo en una de las consolas, al presionar la tecla \cw{enter},
        se enviara a la otra. Si tiene otra computadora conectada en la misma
        red, puede remplazar ``\cw{localhost}'' por la \emph{IP} de la
        computadora donde ejecuto el primer comando. Para salir, presione
        \cw{CTRL-C}.
